...
<body>
    ...
    <div class="container mt-5">
        ...
        <?php
        // Proses Tambah/Update Data Dokter
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                // Update data dokter jika ID ada
                $id = (int)$_POST['id'];
                $sql = "UPDATE dokter SET nama='$nama', alamat='$alamat', no_hp='$no_hp' WHERE id=$id";
                if (mysqli_query($mysqli, $sql)) {
                    echo "<div class='alert alert-success'>Data dokter berhasil diperbarui!</div>";
                } else {
                    echo "<div class='alert alert-danger'>Error: " . mysqli_error($mysqli) . "</div>";
                }
            } else {
                // Tambah data dokter baru jika ID tidak ada
        ...
        if (isset($_GET['hapus'])) {
            ...
            if (mysqli_query($mysqli, $sql)) {
                echo "<div class='alert alert-success'>Data dokter berhasil dihapus!</div>";
            } else {
                echo "<div class='alert alert-danger'>Error: " . mysqli_error($mysqli) . "</div>";
            }
        }

        // Ambil data dokter untuk diubah
        $editData = null;
        if (isset($_GET['edit'])) {
            $id = (int)$_GET['edit'];
        ...
        <table class="table table-striped mt-4">
            ...
            <tbody>
                ...
                while ($data = mysqli_fetch_array($result)) {
                    ...
                    echo "<td>";
                    echo "<a href='dokter.php?edit=".$data['id']."' class='btn btn-success btn-sm'>Ubah</a> ";
                    echo "<a href='dokter.php?hapus=".$data['id']."' class='btn btn-danger btn-sm' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\")'>Hapus</a>";
                    echo "</td>";
                    echo "</tr>";
                }
                ?>
